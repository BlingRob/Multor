...
private:
	...
	struct typeToNum
	{
		static inline int32_t value = (std::is_same_v<type, EShLanguage>) ? 0 :
			(std::is_same_v<type, VkShaderStageFlagBits>) ? 1 :
			(std::is_same_v<type, shader_type>) ? 2 : -1;
	};


	static inline const int32_t typeMatr[NSupportedShaderTypes][NSupportedShaderTypes] = {
													{EShLanguage::EShLangVertex, EShLanguage::EShLangFragment, EShLanguage::EShLangGeometry},
													{VkShaderStageFlagBits::VK_SHADER_STAGE_VERTEX_BIT, VkShaderStageFlagBits::VK_SHADER_STAGE_FRAGMENT_BIT, VkShaderStageFlagBits::VK_SHADER_STAGE_GEOMETRY_BIT},
...
public:
	ShaderLayout() {}
	void addShaderModule(VkShaderModule modul, shader_type type, std::unique_ptr<glslang::TProgram> programm);
...
private:
	...
	void addPipShStInfo(VkShaderModule modul, VkShaderStageFlagBits stage, const char* entryPointName);
	//getShaderVariables();
