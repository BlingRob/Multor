...
public:
    ...
    /// @param msg message
    template <
        typename OutT, typename LogT,
        typename = std::enable_if_t<std::is_base_of_v<LogTypes, LogT> &&
                                    std::is_base_of_v<OutputTarget, OutT> > >
...
private:
    ...
    void log(const char* msg)
    {
        if constexpr (std::is_same_v<OutT, Window>)
            {
                if (pWindow_)
                    SDL_ShowSimpleMessageBox(LogT::SDLvalue, LogT::name, msg,
                                             pWindow_);
                else
                    log<File, LogT>(msg);
            }
