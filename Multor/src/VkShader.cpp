...
void ShaderLayout::addShaderModule(VkShaderModule modul, shader_type type, std::unique_ptr<glslang::TProgram> program)
{
    glslang::TIntermediate* inter = program->getIntermediate(ShaderConverter::convert<EShLanguage>(type));

    addPipShStInfo(modul, ShaderConverter::convert<VkShaderStageFlagBits>(type), inter->getEntryPointName().c_str());
        ...
        Layouts.push_back(VkDescriptorSetLayoutBinding
            {
                ...
                1,
                static_cast<unsigned int>(ShaderConverter::convert<VkShaderStageFlagBits>(type)),
                nullptr
            });
    }
    ...
    for (size_t i = 0; i < program->getNumUniformVariables(); ++i)
    {
        ...
        {
            Layouts.push_back(VkDescriptorSetLayoutBinding{
                    static_cast<uint32_t>(program->getUniform(i).getBinding()),
...
void ShaderLayout::addPipShStInfo(VkShaderModule modul, VkShaderStageFlagBits stage, const char* entryPointName)
{
    VkPipelineShaderStageCreateInfo ShaderStageInfo;

    ShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    ShaderStageInfo.pNext = nullptr;
    ShaderStageInfo.flags = VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT;
...
const std::vector<VkDescriptorSetLayoutBinding>* ShaderLayout::getLayoutBindings()
{
    return &Layouts;
}
