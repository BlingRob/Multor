...
{
	if (std::find(availablePresentModes.cbegin(), availablePresentModes.cend(), VK_PRESENT_MODE_MAILBOX_KHR) != availablePresentModes.cend())
...
void FrameChain::createSwapChain()
{
	...
	uint32_t imageCount = std::min(swapChainSupport.capabilities.minImageCount + 1u,
	...
	uint32_t queueFamilyIndices[] = { physicDevIndices.graphicsFamily.value(), physicDevIndices.presentFamily.value() };
...
void FrameChain::createImageViews()
{
	...
	for (size_t i = 0; i < swapChainImages.size(); ++i)
		...
		swapChainImageViews[i] = MeshFac->createImageView(swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
}
...
void FrameChain::cleanUpSwapChain()
{
	...
	for (auto& imageView : swapChainImageViews)
		vkDestroyImageView(device, imageView, nullptr), imageView = VK_NULL_HANDLE;
