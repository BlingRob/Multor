...
VkBaseStructs::VkBaseStructs(std::shared_ptr<Window> wnd):_pwnd(std::move(wnd)),physicDev(VK_NULL_HANDLE)
{
	CreateInstance();
	CreateSurface();
	InitPhysicalDevice();
...
std::vector<const char*> VkBaseStructs::getRequiredExtensions()
{
	...
	SDL_Vulkan_GetInstanceExtensions(_pwnd->GetWindow().get(), &extensionCount, nullptr);
	extensionNames = new const char* [extensionCount];
	SDL_Vulkan_GetInstanceExtensions(_pwnd->GetWindow().get(), &extensionCount, extensionNames);
