...
std::unique_ptr<glslang::TShader> ShaderFactory::createShader(std::string_view source, EShLanguage type)
{
    EShMessages infoMsg = EShMessages::EShMsgDebugInfo;
    std::unique_ptr<glslang::TShader> shader;
...
std::unique_ptr<glslang::TProgram> ShaderFactory::createProgram(std::unique_ptr<glslang::TShader> shader)
{
    std::unique_ptr<glslang::TProgram> program;
    EShMessages infoMsg = EShMessages::EShMsgDebugInfo;
    //Create glslProgramm
...
std::vector<unsigned int> ShaderFactory::getSPIRV(const glslang::TIntermediate* intr) 
{
    //GLSL -> SPIR-V
...
VkShaderModule ShaderFactory::createModule(const std::vector<unsigned int>& spirv)
{
    //Create VkShaderModul
    VkShaderModuleCreateInfo createInfo{};
    createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
...
std::shared_ptr<ShaderLayout> ShaderFactory::createShader(std::string_view vertex, std::string_view fragment, std::string_view geometry)
{
    ...
    if (!vertex.empty())
    {
        ...
        _pSh->addShaderModule(CreatedModules.back(), shader_type::vertex, std::move(program));
    }
    ...
    if (!fragment.empty())
    {
        ...
        CreatedModules.push_back(createModule(getSPIRV(inter)));
        _pSh->addShaderModule(CreatedModules.back(), shader_type::fragment, std::move(program));
    }
    ...
    if (!geometry.empty())
    {
        ...
        CreatedModules.push_back(createModule(getSPIRV(inter)));
        _pSh->addShaderModule(CreatedModules.back(), shader_type::geometry, std::move(program));
    }
